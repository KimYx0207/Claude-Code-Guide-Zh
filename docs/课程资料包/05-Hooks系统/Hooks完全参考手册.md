...
| Hook类型 | 触发时机 | 典型用途 |
|---------|---------|---------|
| **UserPromptSubmit** | 用户输入提交后 | 提示词优化、敏感词过滤 |
| **PreToolUse** | 工具调用前 | 权限校验、参数验证 |
| **PostToolUse** | 工具调用后 | 格式修复、自动测试 |
| **SessionStart** | 会话开始时 | 环境初始化 |
| **SessionEnd** | 会话结束时 | 清理临时文件 |
| **Stop** | AI停止响应时 | 保存状态 |
| **Notification** | 工具发送通知时 | 日志记录 |
...
| 字段 | 必需 | 说明 | 示例 |
|-----|------|------|------|
| `matcher` | 是 | 匹配的工具名（支持正则） | `"Write"`, `"Write|Edit"` |
| `type` | 是 | Hook类型，固定为`"command"` | `"command"` |
| `command` | 是 | 脚本路径（相对/绝对） | `".claude/hooks/my-hook.py"` |
| `timeout` | 否 | 超时时间（秒），默认10秒 | `5`, `30` |
...
| 字段 | 必需 | 类型 | 说明 |
|-----|------|------|------|
| `decision` | 是 | string | `allow`, `deny`, `ask`, `message` |
| `message` | 否 | string | 显示给用户的消息 |
...
| 环境变量 | 说明 | 示例值 |
|---------|------|--------|
| `CLAUDE_PROJECT_DIR` | 项目根目录 | `C:/Users/admin/Desktop/project` |
| `CLAUDE_SESSION_ID` | 会话ID | `session-12345` |
| `CLAUDE_USER_HOME` | 用户主目录 | `C:/Users/admin` |
