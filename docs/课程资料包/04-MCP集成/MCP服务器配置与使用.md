...
| 传统集成方式 | MCP方式 |
|-------------|---------|
| 每个工具都需要单独对接 | 统一协议，一次对接多处使用 |
| 各家AI平台接口不兼容 | 开放标准，跨平台通用 |
| 安全边界模糊 | 明确的权限控制和隔离机制 |
| 维护成本高 | 生态复用，社区共建 |
...
| 场景 | 推荐作用域 | 原因 |
|------|-----------|------|
| 包含API密钥的配置 | Local | 安全，不会泄露 |
| 团队必需的工具 | Project | 团队共享，版本控制 |
| 个人常用工具 | User | 全局可用，方便 |
| 临时测试 | Local | 不影响其他配置 |
...
| 工具名 | 功能 | 参数 |
|-------|------|------|
| create_entities | 创建实体 | entities |
| create_relations | 创建关系 | relations |
| add_observations | 添加观察 | observations |
| delete_entities | 删除实体 | entity_names |
| delete_observations | 删除观察 | observation_ids |
| delete_relations | 删除关系 | relations |
| read_graph | 读取图 | - |
| search_nodes | 搜索节点 | query |
| open_nodes | 打开节点 | names |
...
| 工具名 | 功能 | 参数 |
|-------|------|------|
| brave_web_search | 执行Web搜索 | query, count |
| brave_local_search | 本地搜索 | query, location |
...
| 工具名 | 功能 | 参数 |
|-------|------|------|
| search | 语义搜索 | query, numResults |
| find_similar | 查找相似内容 | url |
| get_contents | 获取网页内容 | ids |
...
| 工具名 | 功能 | 参数 |
|-------|------|------|
| maps_geocode | 地址编码 | address |
| maps_reverse_geocode | 反向编码 | lat, lng |
| maps_search_places | 搜索地点 | query, location |
| maps_directions | 路线规划 | origin, destination |
...
|-------|------|------|
| git_status | 获取状态 | - |
| git_log | 获取日志 | max_count |
| git_diff | 获取差异 | target |
| git_commit | 创建提交 | message |
| git_add | 暂存文件 | files |
| git_reset | 重置更改 | - |
| git_checkout | 切换分支 | branch |
...
|-------|------|------|
| list_projects | 列出项目 | - |
| get_issue | 获取Issue详情 | issue_id |
| list_issues | 列出Issues | project |
| resolve_issue | 解决Issue | issue_id |
...
| 工具名 | 功能 |
|-------|------|
| s3_list_buckets | 列出S3桶 |
| s3_get_object | 获取S3对象 |
| lambda_invoke | 调用Lambda函数 |
| dynamodb_query | 查询DynamoDB |
...
| 工具名 | 功能 | 参数 |
|-------|------|------|
| puppeteer_navigate | 导航到URL | url |
| puppeteer_screenshot | 截图 | name |
| puppeteer_click | 点击元素 | selector |
| puppeteer_fill | 填充表单 | selector, value |
| puppeteer_evaluate | 执行JavaScript | script |
...
| 工具名 | 功能 | 参数 |
|-------|------|------|
| resolve_library_id | 解析库名到Context7 ID | libraryName |
| get_library_docs | 获取库文档 | context7CompatibleLibraryID, topic, tokens |
...
| 工具名 | 功能 | 参数 |
|-------|------|------|
| sequentialthinking | 执行顺序思考 | thought, nextThoughtNeeded, thoughtNumber, totalThoughts |
...
| reply_to_thread | 回复消息 | channel, thread_ts, text |
| get_channel_history | 获取频道历史 | channel |
| add_reaction | 添加表情反应 | channel, timestamp, name |
...
| 分类 | 服务器名称 | 主要用途 |
|------|-----------|----------|
| **数据** | filesystem, sqlite, postgres, memory | 文件和数据库操作 |
| **搜索** | brave-search, exa, fetch, google-maps | 网络搜索和获取 |
| **开发** | github, gitlab, git, sentry | 代码管理和错误追踪 |
| **云服务** | aws, cloudflare, puppeteer | 云平台和自动化 |
| **知识** | context7, sequential-thinking | 文档查询和推理 |
| **协作** | slack, linear | 团队沟通和项目管理 |
| **自动化** | n8n, make | 工作流自动化 |
| **工具** | time, everything, windows-cli | 系统工具 |
