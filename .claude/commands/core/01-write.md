...
**V7.2.0 (2025-12-10) - 选题过滤器V3集成**：
- 🔥 新增：选题过滤器V3双轨制（核心工具类 vs 泛AI话题类）
- 🔥 新增：产品时效性WebSearch预检（一个月内=热点期）
- ✅ 优化：写作流程集成选题过滤步骤

**V7.1.0 (2025-12-09) - 数据驱动优化**：
- 🔴 修复：删除无效的FOMO公式（82篇零命中）
- 🔴 修复：删除情绪词评分（数据显示0.32x负相关！）
- 🔴 修复：调整标题长度建议（>25字效果更好，非15-25字）
- ✨ 新增：工具推荐公式优先（5.25x effectiveness，最强！）
- ✨ 优化：所有评分规则与rule_validation_report.json数据对齐

**V7.0.0 (2025-12-08)**：
- 命令系统重构：全部重命名为数字前缀
- 新增4个数据驱动工具：标题生成、标题评分、发文前检查、选题过滤
