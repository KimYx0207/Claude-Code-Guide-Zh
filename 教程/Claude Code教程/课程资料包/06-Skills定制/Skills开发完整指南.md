...
**方式**：文件传递
**适用场景**：复杂数据
**示例**：`python script.py --input file.json`


**方式**：环境变量
**适用场景**：配置信息
**示例**：`API_KEY=xxx python script.py`
...
**用途**：正常输出
**格式**：文本/JSON
**示例**：`print(json.dumps(result))`


**通道**：stderr
**用途**：日志/错误
**格式**：文本
**示例**：`logger.error("处理失败")`
...
**脚本**：quality_detector.py
**行数**：643
**功能**：9维度质量检测
**调用命令**：/write, /write-auto


**脚本**：title_generator.py
**行数**：626
**功能**：5公式标题生成
**调用命令**：/write, /title-gen
...
**脚本**：rule_validator.py
**行数**：-
**功能**：规则有效性验证
**调用命令**：/data-analyze


**脚本**：check_data_sync.py
**行数**：-
**功能**：数据同步检查
**调用命令**：内部


**脚本**：collect_wechat_data.py
**行数**：-
**功能**：微信数据收集
**调用命令**：/data-collect


**脚本**：analyze_wechat_data.py
**行数**：-
**功能**：数据分析
**调用命令**：/data-analyze
...
**脚本**：written_article_detector.py
**行数**：-
**功能**：已写文章检测
**调用命令**：内部


**脚本**：fix_article_format.py
**行数**：-
**功能**：格式修复
**调用命令**：内部


**脚本**：migrate_to_database.py
**行数**：-
**功能**：数据迁移
**调用命令**：内部


**脚本**：collect_all_pages.py
**行数**：-
**功能**：全量采集
**调用命令**：/data-collect


**脚本**：collect_incremental.py
**行数**：-
**功能**：增量采集
**调用命令**：/data-collect


**脚本**：collect_time_range.py
**行数**：-
**功能**：时间范围采集
**调用命令**：/data-collect


**脚本**：browser_console_extract.js
**行数**：-
**功能**：浏览器提取
**调用命令**：内部


**脚本**：remove_profanity.js
**行数**：-
**功能**：脏话过滤
**调用命令**：内部
...
**维度**：品牌词
**最高分**：35
**有效性(effectiveness)**：1.59x


**维度**：动作词
**最高分**：15
**有效性(effectiveness)**：1.95x


**维度**：效率词
**最高分**：10
**有效性(effectiveness)**：1.68x


**维度**：工具推荐公式
**最高分**：20
**有效性(effectiveness)**：5.25x


**维度**：问题解决
**最高分**：10
**有效性(effectiveness)**：1.65x


**维度**：数字/版本
**最高分**：10
**有效性(effectiveness)**：1.42x


**维度**：长度
**最高分**：5
**有效性(effectiveness)**：(反向:短标题负相关)
...
**快速检查命令**：`ls -R .claude/skills/`
**解决方案**：修正skill.yaml中的路径
...
**快速检查命令**：`ls -la`
**解决方案**：`chmod +x script.py`


**问题**：**JSON解析失败**
**症状**：JSONDecodeError
**快速检查命令**：`python -m json.tool`
**解决方案**：检查JSON格式


**问题**：**YAML语法错误**
**症状**：YAMLError
**快速检查命令**：`python -c "import yaml"`
**解决方案**：使用YAML验证器
...
**协议**：**MIT**
**特点**：最宽松，允许商业使用，只需保留版权声明
**适用场景**：希望最大化传播，不在意商业使用
...
**章节**：**第一章**
**核心内容**：高级开发技术
**关键技能**：多步骤工作流、状态管理、错误恢复、DAG依赖、并行执行


**章节**：**第二章**
**核心内容**：Scripts集成
**关键技能**：Python规范、参数传递、stdin/stdout、性能优化、调试技巧


**章节**：**第三章**
**核心内容**：领域Skill设计
**关键技能**：技术写作、代码审查、数据分析、模板化设计


**章节**：**第四章**
**核心内容**：项目脚本分析
**关键技能**：20个脚本深度剖析、数据驱动、规则演进


**章节**：**第五章**
**核心内容**：调试与发布
**关键技能**：10大问题排查、5大性能优化、发布规范


**章节**：**第六章**
**核心内容**：完整实战案例
**关键技能**：论文摘要器、代码检查器、从零到一实战
